...
//@Query("SELECT d, c  FROM   Driver d, CarrierAssignDriver c  INNER JOIN Driver ON d.driverId = c.driverId WHERE c.bidding=:biddingId")
  ...
  @Query(value ="SELECT drivers.driver_id,drivers.first_name,drivers.last_name,drivers.middle_name,drivers.phone_number, assign_drivers.bidding_id FROM    assign_drivers INNER JOIN drivers ON drivers.driver_id =assign_drivers.driver_id WHERE  assign_drivers.bidding_id = :biddingId",nativeQuery = true)
   List<Object> getDriverforAnOrder(@Param("biddingId")  int biddingId);
