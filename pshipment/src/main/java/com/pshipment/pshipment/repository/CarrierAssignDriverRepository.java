...
public interface CarrierAssignDriverRepository extends JpaRepository<CarrierAssignDriver, Integer> {
  ...
  //@Query("SELECT new com.pshipment.pshipment.dto.DriversforAnOrder(d.driverId,d.firstName,d.middleName,d.lastName,d.phoneNumber) FROM CarrierAssignDriver c JOIN c.driverId d JOIN c.bidding b  WHERE c.bidding=:biddingId")
  ...
  @Query(name = "findDriversofAnOrder", nativeQuery = true)
   List<DriversforAnOrderDto> getDriverforAnOrder(@Param("biddingId")  Integer biddingId);
